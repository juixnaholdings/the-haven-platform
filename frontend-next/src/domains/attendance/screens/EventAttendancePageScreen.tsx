...
function getAttendanceProgressTone (
...
export function EventAttendancePageScreen () {
  ...
  const [summaryFormOverrides, setSummaryFormOverrides] = useState<
    Partial<SummaryFormState>
  >({})
  const [addFormState, setAddFormState] = useState<MemberAttendanceFormState>(
    emptyMemberAttendanceForm
  )
  const [selectedMemberAttendanceId, setSelectedMemberAttendanceId] = useState<
    number | null
  >(null)
  const [editFormOverrides, setEditFormOverrides] = useState<
    Partial<MemberAttendanceEditFormState>
  >({})
  ...
  const memberAttendanceQuery = useQuery({
    ...
    queryKey: [
      ...
      { search: deferredMemberSearch, statusFilter }
    ],
    queryFn: () =>
  ...
  const existingMemberIds = useMemo(
    () =>
      new Set(
        (serviceEventQuery.data?.member_attendances ?? []).map(
          memberAttendance => memberAttendance.member_id
        )
      ),
  ...
  const baseEditFormState = useMemo<MemberAttendanceEditFormState>(() => {
    ...
    return {
      ...
      checked_in_at: toDateTimeLocalInputValue(
        selectedMemberAttendance.checked_in_at
      ),
  ...
  const createMemberAttendanceMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({
        queryKey: ['member-attendance', numericServiceEventId]
      })
      await queryClient.invalidateQueries({ queryKey: ['service-events'] })
      await queryClient.invalidateQueries({ queryKey: ['attendance-overview'] })
  ...
  if (
    serviceEventQuery.error ||
    memberAttendanceQuery.error ||
    ...
    <div className='space-y-6'>
      <PageHeader
        ...
        meta={
          <>
            <StatusBadge
              label={getServiceEventTypeLabel(serviceEvent.event_type)}
              tone='info'
            />
            <StatusBadge
            ...
            <StatusBadge
              ...
              tone={getAttendanceProgressTone(
                serviceEvent.attendance_progress_status
              )}
            />
          </>
        }
      ...
      <section className='rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-sm'>
        <div className='section-header'>
          <div>
            ...
            <p className='m-0 text-sm text-slate-500'>
              Anonymous summary counts remain separate from member rows in the
        ...
        <dl className='definition-list'>
          ...
          <div>
            ...
            <dd>
              {formatDateTime(serviceEvent.attendance_summary?.updated_at)}
            </dd>
          </div>
        </dl>
      </section>
      ...
      <section className='rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-sm'>
        <div className='section-header'>
          <div>
            ...
            <p className='m-0 text-sm text-slate-500'>
              Filter and manage per-member attendance records for this event.
            </p>
          </div>
        </div>
            ...
            <EntityTable
              columns={[
                  ...
                  cell: memberAttendance => (
                    <div className='grid gap-1'>
                      <Link
                        ...
                        href={`/members/${memberAttendance.member_id}`}
                      >
                        {formatMemberName(memberAttendance)}
                      </Link>
                      <span className='block text-xs text-slate-500'>
                        {memberAttendance.email || 'Profile-only record'}
                      </span>
                    </div>
                  )
                },
                ...
                },
                {
                  header: 'Checked in',
                  cell: memberAttendance =>
                    formatDateTime(memberAttendance.checked_in_at)
                },
                {
                  header: 'Last updated',
                  cell: memberAttendance =>
                    formatDateTime(memberAttendance.updated_at)
                },
                {
                  header: 'Actions',
      ...
      <section className='rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-sm'>
        ...
        {selectedMemberAttendance ? (
          <p className='m-0 text-sm text-slate-500'>
            Selected for edit:{' '}
            <strong>{formatMemberName(selectedMemberAttendance)}</strong>
          </p>
        ) : (
          <p className='m-0 text-sm text-slate-500'>
      ...
      >
        <form
          ...
          onSubmit={event => {
            event.preventDefault()
            createMemberAttendanceMutation.mutate(
              toMemberAttendancePayload(addFormState)
            )
          }}
        >
          <FormSection title='Member attendance record'>
            ...
            <p className='m-0 text-sm text-slate-500'>
              {candidateMembersQuery.isLoading
                ...
                ? `${candidateMembers.length} eligible member${
                    candidateMembers.length === 1 ? '' : 's'
                  } found.`
                : 'No eligible members match the current search.'}
            </p>
            <p className='m-0 text-sm text-slate-500'>
              Duplicate prevention is active: each member can only have one
              attendance record per event.
            </p>
          </FormSection>
      ...
      <FormModalShell
        ...
        title={
          selectedMemberAttendance
            ? `Edit attendance record: ${formatMemberName(
                selectedMemberAttendance
              )}`
      ...
      >
        <form
          ...
          onSubmit={event => {
            event.preventDefault()
            updateMemberAttendanceMutation.mutate(
              toMemberAttendanceUpdatePayload(editFormState)
            )
          }}
        >

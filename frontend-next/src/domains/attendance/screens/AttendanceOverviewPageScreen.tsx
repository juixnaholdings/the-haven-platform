...
export function AttendanceOverviewPageScreen () {
  ...
  const incompleteEvents = serviceEvents.filter(
    serviceEvent => !serviceEvent.attendance_is_complete
  )
  const completedEvents = serviceEvents.filter(
    serviceEvent => serviceEvent.attendance_is_complete
  )
          ...
          <div className='section-header'>
            <div>
              ...
              <p className='m-0 text-sm text-slate-500'>
                The backend intentionally keeps the two attendance surfaces
                separate.
              </p>
            </div>
          </div>
          <ul className='item-list'>
            <li className='item-row'>
              <div>
                ...
                <span>
                  Aggregate headcounts across men, women, children, and
                  visitors.
                </span>
              </div>
            </li>
            <li className='item-row'>
              <div>
                ...
                <span>
                  Per-member status records used during the event attendance
                  workflow.
                </span>
              </div>
            </li>
        ...
        <section className='rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-sm'>
          <div className='section-header'>
            <div>
              ...
              <p className='m-0 text-sm text-slate-500'>
                Anonymous summary totals across the selected date range.
              </p>
            </div>
          </div>
        ...
        <EmptyState
          ...
          title={
            hasDateFilter
              ...
              : 'No events available for attendance yet'
          }
        />
      ) : (
        <section className='rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-sm'>
          <div className='section-header'>
            <div>
              <h3>Recent events</h3>
              <p className='m-0 text-sm text-slate-500'>
                Open the event detail or direct attendance recording workflow
                from here.
              </p>
            </div>
          </div>
                ...
                cell: serviceEvent => (
                  <div className='grid gap-1'>
                    ...
                    <span className='block text-xs text-slate-500'>
                      {getServiceEventTypeLabel(serviceEvent.event_type)} |{' '}
                      {formatDate(serviceEvent.service_date)}
                    </span>
                  </div>
                )
              },
              {
                ...
                cell: serviceEvent => (
                  <div className='grid gap-1'>
                    ...
                    <span className='block text-xs text-slate-500'>
                      Last update:{' '}
                      {formatDate(serviceEvent.attendance_last_updated_at)}
                    </span>
                  </div>
                )
              },
              ...
              },
              {
                ...
                cell: serviceEvent => (
                  <div className='flex flex-wrap items-center gap-2.5'>
                    ...
                    >
                      {serviceEvent.attendance_is_complete
                        ? 'Review'
                        : 'Continue'}
                    </Link>
                  </div>
          ...
          <div className='section-header'>
            <div>
              ...
              <p className='m-0 text-sm text-amber-900/80'>
                Continue these events to complete summary and member-level
                attendance capture.
              </p>
            </div>
            <StatusBadge
              label={`${incompleteEvents.length} pending`}
              tone='warning'
            />
          </div>
          <ul className='item-list'>
            {incompleteEvents.slice(0, 6).map(serviceEvent => (
              <li className='item-row' key={serviceEvent.id}>
                <div>
                  ...
                  <span>
                    {formatDate(serviceEvent.service_date)} |{' '}
                    {serviceEvent.attendance_progress_label}
                  </span>
                </div>
                <Link
                  className='button button-secondary button-compact'

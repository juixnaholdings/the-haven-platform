...
import { ErrorAlert, FormModalShell, FormSection, StatusBadge } from "@/components";
import { attendanceApi } from "@/domains/attendance/api";
import { ATTENDANCE_STATUS_OPTIONS, SERVICE_EVENT_TYPE_OPTIONS } from "@/domains/attendance/options";
...
export function RecordAttendanceModal({
  ...
  description = "Capture event context and attendance in one modal workflow. The system creates the event first, then records attendance against that event.",
}: RecordAttendanceModalProps) {
  const [eventFormState, setEventFormState] = useState<EventFormState>(() => getDefaultEventFormState());
  const [summaryFormState, setSummaryFormState] = useState<SummaryFormState>(() => getDefaultSummaryFormState());
  const [memberSearch, setMemberSearch] = useState("");
  const [memberDraftState, setMemberDraftState] = useState<MemberAttendanceDraftState>(
    () => getDefaultMemberAttendanceDraftState(),
  );
  const [memberDraftRows, setMemberDraftRows] = useState<MemberAttendanceDraft[]>([]);
  ...
  const memberOptions = useMemo(
    () =>
      (membersQuery.data ?? []).filter((member) => !selectedMemberIds.has(member.id)),
  ...
  const workflowMutation = useMutation({
    mutationFn: async (): Promise<RecordAttendanceWorkflowResult> => {
      const createdEvent = await attendanceApi.createServiceEvent(toEventPayload(eventFormState));
      let summarySaved = false;
      let memberRowsSaved = 0;

      try {
        await attendanceApi.upsertAttendanceSummary(createdEvent.id, summaryPayload);
        summarySaved = true;

        for (const row of memberDraftRows) {
          await attendanceApi.createMemberAttendance(createdEvent.id, toMemberAttendancePayload(row));
          memberRowsSaved += 1;
        }
      } catch (error) {
  ...
  const isSubmitDisabled =
    workflowMutation.isPending || !eventFormState.title.trim() || !eventFormState.service_date;
  ...
  return (
      ...
      footer={
        <>
          ...
          <button
            ...
            type="submit"
          >
            {workflowMutation.isPending ? "Recording..." : "Create event and record attendance"}
          </button>
        </>
      }
      isOpen={isOpen}
    ...
    >
      ...
      >
        <section className="rounded-2xl border border-slate-200/80 bg-slate-50/60 p-4 text-sm text-slate-600">
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge label="Today" tone="info" />
            <span>{formatDate(new Date().toISOString())}</span>
          </div>
          <p className="mb-0 mt-2">
            This workflow creates the event first, then saves summary and member attendance rows.
          </p>
        </section>

        <FormSection
          description="Set event details for the attendance session."
          title="Event details"
        >
        ...
        >
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <div className="rounded-2xl border border-slate-200/70 bg-slate-50/60 p-4">
              <dt>Derived total</dt>
              <dd className="m-0 mt-1 text-lg font-semibold text-slate-900">{summaryPayload.total_count}</dd>
            </div>
          </div>
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <button
              ...
              onClick={() => {
                ...
                setMemberDraftRows((current) => [
                  ...current,
                  {
                    client_id: `${memberDraftState.member_id}-${Date.now()}-${current.length}`,
                    ...memberDraftState,
                  },
                ]);
                setMemberDraftState((current) => ({
            ...
            <p className="m-0 text-sm text-slate-500">
              {membersQuery.isLoading
                ? "Loading members..."
                : `${memberOptions.length} eligible member${memberOptions.length === 1 ? "" : "s"} available.`}
            </p>
          </div>
          ...
          {memberDraftRows.length > 0 ? (
              ...
              {memberDraftRows.map((row) => {
                ...
                return (
                  <li className="item-row" key={row.client_id}>
                    <div>
                      <strong>{member?.full_name || `Member #${row.member_id}`}</strong>
                      <span>
                        {ATTENDANCE_STATUS_OPTIONS.find((option) => option.value === row.status)?.label || row.status}
                    ...
                    <button
                      ...
                      onClick={() => {
                        setMemberDraftRows((current) =>
                          current.filter((entry) => entry.client_id !== row.client_id),
                        );
                      }}
                      type="button"
                    >
            ...
            <p className="m-0 text-sm text-slate-500">No member rows added yet.</p>
          )}
        </FormSection>
        ...
        {workflowError ? (
          <section className="rounded-2xl border border-amber-300/80 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            ...
            <p className="mb-0 mt-2">
              Summary saved: {workflowError.summarySaved ? "Yes" : "No"} | Member rows saved: {workflowError.memberRowsSaved}
            </p>
            <p className="mb-0 mt-2">
              Continue from the event attendance page:
              {" "}
              <Link className="font-semibold underline" href={`/events/${workflowError.eventId}/attendance`}>

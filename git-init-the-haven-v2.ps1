...
if ($LASTEXITCODE -ne 0) {
    Invoke-Git -GitArgs @("branch", "develop")
}

Write-Step "Optional example branches"
if ($FeatureExample) {
    & git show-ref --verify --quiet "refs/heads/feat/$FeatureExample"
    if ($LASTEXITCODE -ne 0) { Invoke-Git -GitArgs @("branch", "feat/$FeatureExample", "develop") }
}
if ($BugfixExample) {
    & git show-ref --verify --quiet "refs/heads/bugfix/$BugfixExample"
    if ($LASTEXITCODE -ne 0) { Invoke-Git -GitArgs @("branch", "bugfix/$BugfixExample", "develop") }
}
if ($RefactorExample) {
    & git show-ref --verify --quiet "refs/heads/refactor/$RefactorExample"
    if ($LASTEXITCODE -ne 0) { Invoke-Git -GitArgs @("branch", "refactor/$RefactorExample", "develop") }
}
if ($HotfixExample) {
    & git show-ref --verify --quiet "refs/heads/hotfix/$HotfixExample"
    if ($LASTEXITCODE -ne 0) { Invoke-Git -GitArgs @("branch", "hotfix/$HotfixExample", "main") }
}
...
if ($RemoteUrl) {
    ...
    if ($Push) {
        ...
        Invoke-Git -GitArgs @("push", "-u", "origin", "develop")

        if ($FeatureExample) { Invoke-Git -GitArgs @("push", "-u", "origin", "feat/$FeatureExample") }
        if ($BugfixExample)  { Invoke-Git -GitArgs @("push", "-u", "origin", "bugfix/$BugfixExample") }
        if ($RefactorExample){ Invoke-Git -GitArgs @("push", "-u", "origin", "refactor/$RefactorExample") }
        if ($HotfixExample)  { Invoke-Git -GitArgs @("push", "-u", "origin", "hotfix/$HotfixExample") }
    }
}

Invoke-Git -GitArgs @("checkout", "develop")
